...
1. **Editor** (`app/splits/*`, `components/editor`, `components/splits`). CRUD over the nested model — build a routine, set its targets, and assign it to a client. Trainer-only (the API 403s a client on write). Reached from the header link, not from the home dashboard. `ExerciseRow` renders targets through `toPlanExercise` *and* numbers rows through `toSheetItems`, so the editor shows the exact string and the exact number (`02A`) the trainee will read — which is also what exposes a superset whose members drifted apart.
...
- `hooks/use-reorder.ts` + `lib/reorder.ts` — position is a per-item `order` and the API has no reorder endpoint, so moving something is N `PATCH`es. `reorder()` renumbers from the list's **existing minimum** (days/exercises are 0-based, microcycles 1-based because a microcycle's `order` *is* its week number) and returns only the items that changed; `applyOrders()` writes them into the cached `Split` for the optimistic repaint. There is no manual "Orden" input in the dialogs — position is the arrows' job.
